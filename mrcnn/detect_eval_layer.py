...
from mrcnn.utils        import apply_box_deltas_np, non_max_suppression, logt, flip_bbox, overlaps_graph_np
...
def add_evaluation_detections_2(gt_class_ids, gt_bboxes, config):
    '''
    ...
    for class_id in np.unique(gt_nz_class_ids) :
        ...
        cls_avg_score = config.class_prediction_avg[class_id] ## np.average(dt_nz[dt_class_ixs,5])
...
class DetectionEvaluateLayer(KE.Layer):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        print('\n>>> Detection Layer (Evaluation Mode)')
        self.config = config
    ...
    def call(self, inputs):
        ...
        def wrapper(rois, mrcnn_class, mrcnn_bbox, image_meta, gt_class_ids, gt_bboxes):
            ...
            for b in range(self.config.BATCH_SIZE):
                ...
                mod_detections, max_overlap  =  add_evaluation_detections_2( gt_class_ids[b], gt_bboxes[b], self.config)
                if self.verbose:
