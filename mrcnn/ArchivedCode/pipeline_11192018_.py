...
##------------------------------------------------------------------------------------
def obsolete_coco_dataset(type, config):
    '''
        type = { train, val, test}
    '''
...
##------------------------------------------------------------------------------------
def prep_coco_dataset(type, config, generator = False, shuffle = True, augment = False):
...
def display_training_batch(dataset, batch_x, masks= False):
    ...
    for img_idx in range(img_meta.shape[0]):
        ...
        if masks:
            visualize.display_instances_with_mask(image, bbox, mask, class_ids, dataset.class_names) 
        else:
...
def get_training_batch(dataset, config, image_ids, display = True, masks = False):
    ...
    if display:
        display_training_batch(dataset, batch_x, masks = masks)
...
def get_inference_batch(dataset, mrcnn_model, image_ids, display = False):
    ...
    if display:
        ...
        # visualize.display_images(images, titles = titles)        
        display_training_batch(dataset, [molded_images, image_metas], masks = True)
