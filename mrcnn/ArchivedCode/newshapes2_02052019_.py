...
class NewShapesDataset(Dataset):
    ...
    def load_shapes(self, count, buffer = 20):
        ...
        print('Active Class Info')
        print('-----------------')
        print(self.class_info)
        
        self.object_choices = [ cls_inf['name'] for cls_inf in self.class_info if cls_inf['id'] > 0]
        print('Objct Choices')
        print('-----------------')
        print(self.object_choices)
        ...
        for i in range(count):
            # if i % 25 == 0:
                # print(' Add image ---> ',i )
            bg_color, shapes = self.build_image(i, height, width)
    ...
    ## build_image
    ##---------------------------------------------------------------------------------------------
    def build_image(self, image_id, height, width):
        '''
        ...
        # object_choices = ["person", "car", "sun", "building", "tree", "cloud"]
        object_choices = copy.deepcopy(self.object_choices)
        
        for _ in range(N):
            shape = random.choice(object_choices)            
            color, dims = self.build_object(shape, height, width)
            shapes.append((shape, color, dims))
            if shape == "sun":
                print('remove sun')
                object_choices.remove("sun")
                print('object_choices: ', object_choices)
                print('self.object_choices: ', self.object_choices)
...
#             print('X :', x, 'y:', y , '     sx: ',sx , 'sy: ', sy, 'hs:', hs)
    ...
    ## build_object
    ##---------------------------------------------------------------------------------------------
    def build_object(self, shape, height, width):
