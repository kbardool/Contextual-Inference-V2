...
def fcn8_graph(feature_map , config):
    ...
    x  = KL.Deconvolution2D(num_classes,kernel_size=(4,4), strides = (2,2),
    ...
    upscore_pool4 = KL.Deconvolution2D(num_classes, kernel_size=(4,4), strides = (2,2), 
                                       padding = "same", activation = None, name = "fcn16_upscore_pool4")(x)
    ...
    scorePool3 = KL.Conv2D(num_classes, (1,1), activation="relu", padding="valid", 
                           kernel_initializer='glorot_uniform',  bias_initializer='zeros', 
                           kernel_regularizer=l2(weight_decay), name="fcn8_score_pool3")(Pool3)                    
    print()
    print('   --- FCN8 ----------------------------')
    ...
    ##-------------------------------------------------------------------------------------------------------    
    # Upsampling  (padding was originally "valid", I changed it to "same" )
    x = KL.Deconvolution2D(num_classes, kernel_size=(16,16), strides = (8,8),
                           kernel_initializer='glorot_uniform', bias_initializer='zeros',
                           padding = "same", activation = None, name = "fcn8_classify")(x)
    print()
    print('   FCN fcn8_classify  (Deconv(fuse_Pool4)) shape is     : ' , KB.int_shape(x),
          '   keras_tensor ', KB.is_keras_tensor(x))                      
