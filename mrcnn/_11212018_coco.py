...
class CocoDataset(dataset.Dataset):
    ...
    def load_coco(self, dataset_dir, subset, load_class_ids=None,
...
#       image_dir = os.path.join(dataset_dir, "train2017" if subset == "train" lse "val2017")
        ...
        # if load_class_ids:
            # print(' load subset of classes: ', load_class_ids)
            # image_ids = []
            # for id in load_class_ids:
                # image_ids.extend(list(coco.getImgIds(catIds=[id])))
            # # Remove duplicates
            # image_ids = list(set(image_ids))
        # else:
            # # load All image ids
            # print(' load all classes: ', load_class_ids)            
            # image_ids = list(coco.imgs.keys())
...
##------------------------------------------------------------------------------------
def prep_coco_dataset(type, config, load_class_ids = None, class_ids = None, 
    ...
    for i in type:
        dataset.load_coco(config.COCO_DATASET_PATH, i , class_ids = class_ids, 
                            load_class_ids = load_class_ids, return_coco = return_coco)
