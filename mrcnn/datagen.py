...
def load_image_gt(dataset, config, image_id, augment=False, use_mini_mask=False):
    ...
    Mask only used for BBOX determinination and is not returned from function
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    #   so we train a maximum of 128 positive and negative anchors, the rest 
    #   are set to netural.
    #--------------------------------------------------------------------------
    ids = np.where(rpn_match == 1)[0]
    extra = len(ids) - (config.RPN_TRAIN_ANCHORS_PER_IMAGE // 2)
    # print( 'build_rpn_targets() : Positives: {}, Extra: {} '.format(len(ids), extra))
    ...
    # print( 'build_rpn_targets() : Negatives: {}, Extra: {} '.format(len(ids), extra))
    if extra > 0:
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
                   batch_size=1, detection_targets=False, image_index = -1):
    '''
...
##----------------------------------------------------------------------
def data_gen_simulate(dataset, config, image_index):
    '''
