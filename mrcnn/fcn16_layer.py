...
def fcn16_graph(feature_map , config , weight_decay=0.0002, batch_momentum=0.9):
    ...
    x = KL.Conv2D(num_classes, (1, 1), kernel_initializer='he_normal',  bias_initializer='zeros', 
                  ...
                  name='fcn32_classify')(x)
    print('   FCN conv2d (fcn32_classify) shape is : ' , x.get_shape(),' keras_tensor ', KB.is_keras_tensor(x))                      
    
    ## 2x Upsampling to generate Score2 (padding was originally "valid")
    score2 = KL.Deconvolution2D(num_classes,kernel_size=(4,4),strides = (2,2),padding = "same",activation=None,name = "score2")(x)
    print('\n   FCN 2x Upsampling (Deconvolution2D(fcn32_classify)) shape is : ' , KB.int_shape(score2),' keras_tensor ', KB.is_keras_tensor(score2))                      
    
    ## Score Pool4
    scorePool4 = KL.Conv2D(num_classes, (1,1), activation='relu', padding='same', name='score_pool4',
                          kernel_initializer='glorot_uniform', bias_initializer='zeros', kernel_regularizer=l2(weight_decay))(Pool4)                    
    print('\n   FCN scorePool4 (Conv2D(Pool4)) shape is : ' , KB.int_shape(scorePool4),' keras_tensor ', KB.is_keras_tensor(scorePool4))                      
                  
    ## Sum Score2, Pool4
    x = KL.Add(name='fuse_pool4')([score2,scorePool4])    
    print('\n   FCN Add Score2,scorePool4 shape is : ' , x.get_shape(),' keras_tensor ', KB.is_keras_tensor(x))                      

    ## Upsampling  (padding was originally "valid")
    fcn_hm = KL.Deconvolution2D(num_classes,kernel_size=(32,32),strides = (16,16),padding = "same",activation = None,name = "fcn_heatmap")(x)
    print('\n   FCN fcn16_classify (Deconv(fuse_Pool4)) shape is : ' , KB.int_shape(fcn_hm),' keras_tensor ', KB.is_keras_tensor(fcn_hm))                      
